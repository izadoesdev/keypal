...
import type { DrizzleColumnMapping, Storage } from "../types/storage-types";
...
export class DrizzleStore implements Storage {
	...
	private rowToRecord(row: any): ApiKeyRecord {
		return {
			...
			metadata: {
				...
				scopes: row[this.columns.scopes]
					? JSON.parse(row[this.columns.scopes])
					: undefined,
				resources: row[this.columns.resources]
					? JSON.parse(row[this.columns.resources])
					: undefined,
	...
	private recordToRow(record: ApiKeyRecord): any {
		return {
			...
			[this.columns.scopes]: record.metadata.scopes
				? JSON.stringify(record.metadata.scopes)
				: null,
			[this.columns.resources]: record.metadata.resources
				? JSON.stringify(record.metadata.resources)
				: null,
	...
	async updateMetadata(
		...
		if (metadata.resources !== undefined)
			updates[this.columns.resources] = JSON.stringify(metadata.resources);
		...
		if (metadata.enabled !== undefined)
			updates[this.columns.enabled] = metadata.enabled;
		if (metadata.revokedAt !== undefined)
			updates[this.columns.revokedAt] = metadata.revokedAt;
		if (metadata.rotatedTo !== undefined)
			updates[this.columns.rotatedTo] = metadata.rotatedTo;
	...
	async delete(id: string): Promise<void> {
		await this.db.delete(this.table).where(eq(this.table[this.columns.id], id));
	}
