...
import { createAdapterFactory, type SchemaConfig } from "./adapter-factory";
import { DEFAULT_QUERY_LIMIT, calculateLogStats } from "./utils";

export { apikey as DrizzleApiKeySchema } from "../drizzle/schema";
		...
		adapter: (context) => {
			...
			return {
				...
				async findByTags(
					...
					// biome-ignore lint/suspicious/noExplicitAny: arrayContains returns SQL[]
				...
				...(auditLogTable &&
					context.transformAuditLogInput && {
						...
						async findLogs(query: AuditLogQuery): Promise<AuditLog[]> {
							...
							const timestampCol = context.getColumnName(
								"auditlog",
								"timestamp"
							);
...
export class DrizzleStore implements Storage {
	...
	findByTags = (tags: string[], ownerId?: string) =>
		this.storage.findByTags(tags, ownerId);
	findByTag = (tag: string, ownerId?: string) =>
		this.storage.findByTag(tag, ownerId);
	update = (id: string, fields: Partial<ApiKeyMutableFields>) =>
		this.storage.update(id, fields);
	...
	saveLog = (log: AuditLog) =>
		this.storage.saveLog?.(log) ?? Promise.resolve();
	findLogs = (query: AuditLogQuery) =>
		this.storage.findLogs?.(query) ?? Promise.resolve([]);
	countLogs = (query: AuditLogQuery) =>
		this.storage.countLogs?.(query) ?? Promise.resolve(0);
	deleteLogs = (query: AuditLogQuery) =>
		this.storage.deleteLogs?.(query) ?? Promise.resolve(0);
	getLogStats = (ownerId: string) =>
		this.storage.getLogStats?.(ownerId) ??
		Promise.resolve({ total: 0, byAction: {}, lastActivity: null });
}
